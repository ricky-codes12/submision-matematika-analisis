...
Original file is located at
    https://colab.research.google.com/drive/1hca03CU7-krrxqa-QArcBH6OusqGcBgF
...
def estimate_sample_size_proportions(p1, p2, alpha=0.05, beta=0.20, two_sided=True):
    ...
    n = (
        (
            z_alpha * math.sqrt(p_bar * q_bar * (1 + 1/k)) +
            z_beta * math.sqrt(p1 * q1 + (p2 * q2) / k)
        ) ** 2
    ) / (delta ** 2)
    # AKHIRI KODE DI SINI
...
def generate_df_ab_test(n_days):
    ...
    # Split user_ids into origin and varian groups
    user_ids_origin = user_ids[:n_origin]
    user_ids_varian = user_ids[n_origin:]
    ...
    origin_dict = {
        'user_id': user_ids_origin,
        'user_type': ['origin'] * n_origin,
        'converted': data_origin
    }
    varian_dict =  {
        'user_id': user_ids_varian,
