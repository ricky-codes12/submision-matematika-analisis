...
Original file is located at
    https://colab.research.google.com/drive/1pUKIIbGXbEOqWSypF9gKqOeozRdviUT_
...
def viz_uniform_teoretis(lower_bound, upper_bound, num_samples):
  ...
  data_uniform = generate_rand_uniform(
...
def generate_gaussian(miu, sigma, num_samples):
  ...
  uniform_samples = generate_rand_uniform(
        lower_bound=0.0,
        upper_bound=1.0,
        num_samples=num_samples
  )
  ...
  array = uniform_inverse_cdf(
        probability=uniform_samples,
        miu=miu,
        sigma=sigma
   )
...
def generate_data_synthetic(breed, features, n_samples, params):
  ...
  for feature in features:
    ...
    if feature in ["wingspan_cm", "weight_g"]:  # Gaussian
        df[feature] = generate_gaussian(
            miu=params[breed][feature].miu,
            sigma=params[breed][feature].sigma,
            num_samples=n_samples
        )

    elif feature == "sing_days":  # Binomial
        df[feature] = generate_binomial(
            n_trials=params[breed][feature].n_trials,
            success_prob=params[breed][feature].probability,
            num_samples=n_samples
        )

    elif feature == "beak_head_ratio":  # Uniform
        ...
        raise ValueError(f"Fitur {feature} tidak dikenal!")
...
def gaussian_params_estimation(sample):
  ...
  # MULAI KODE DI SINI
  miu = np.mean(sample)
  sigma = np.std(sample, ddof=0)  # ddof=0 untuk populasi
  # AKHIRI KODE DI SINI
...
def training_params_estimation(df, features):
    ...
    # Iterasi pada setiap kelas/breed
    for breed in sorted(df['breed'].unique()):
        df_breed = df[df['breed'] == breed][features]

        # proporsi tiap kelas
        probs_dict[int(breed)] = len(df_breed) / len(df)
        ...
        for feature in df_breed.columns:
            if feature in ['wingspan_cm', 'weight_g']:
                ...
                sigma = df_breed[feature].std(ddof=1)  # standar deviasi sampel
                params = gaussian_params(miu=mu, sigma=sigma)

            elif feature == 'sing_days':
                # Gunakan n_trials dan probability sesuai data sintetis
                if breed == 0:
                    n_trials = 29
                    probability = 0.800
                elif breed == 1:
        ...
        params_dict[int(breed)] = inner_dict

    # Kembalikan dictionary parameter dan probabilitas
...
def probability_given_class(X, features, breed, params_dict):
    ...
    for x_val, feature in zip(X, features):
        params = params_dict[breed][feature]

        if feature in ['wingspan_cm', 'weight_g']:
            # Gaussian PDF
            probability_f = norm.pdf(x_val, loc=params.miu, scale=params.sigma)

        elif feature == 'sing_days':
            # Binomial PMF
            probability_f = binom.pmf(int(x_val), n=params.n_trials, p=params.probability)

        elif feature == 'beak_head_ratio':
            # Uniform PDF
            probability_f = uniform.pdf(x_val, loc=params.lower_bound, scale=(params.upper_bound - params.lower_bound))
